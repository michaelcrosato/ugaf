  ...
  },
  {
    ...
    look: {
      ...
      variants: [
        ...
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { fact: 'flag.knows_gap', eq: true }] }, text: 'You know this gate now, the way Holt told it: the wedge of dark by the north post where the floodlight throws crooked. Go low and quiet there (HIDE) while the troopers are not roused, and you can be through it before they think to look.' },
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { not: { fact: 'flag.knows_gap', eq: true } }, { fact: 'reputation.pc.striders', lte: 0 } ] }, text: 'You do not know this gate\'s rhythm — where its light falls short, or when. Holt might mutter it, in a talkative hour: ask him about the gap. Failing that, good iron would lever the wire-gap wide, or a Strider\'s debt would walk you out.' },
    ...
    exits: [
      {
        ...
        // not enough — slipping out needs the gate's blind spot (ask Holt about the gap) AND a calm
        // patrol; or working iron to lever the gap; or a Strider's debt.
        when: { any: [{ not: { fact: 'flag.intercepted', eq: true } }, { fact: 'flag.intercept_clear', eq: true }, { fact: 'reputation.pc.striders', gte: 1 }, { all: [{ fact: 'flag.knows_gap', eq: true }, { fact: 'flag.hidden', eq: true }, { fact: 'awareness.cordon_patrol', neq: 'alert' }] }] },
        blockedText:
          'The boom gate is down and the troopers are at the wire, watching for exactly what rides in your pack. You will not simply walk the core out under their noses. To slip past you must know where the floodlight falls short — ask Warden Holt about the gap, then go low and quiet (HIDE) while they are not roused. Or lever the wire-gap wide with good iron (USE your bar or knife — if the Greywater has not eaten its temper). Or lean on a debt, if the Striders owe you one.',
      },

  ...
  },
  {
    ...
    look: {
      ...
      variants: [
        ...
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { fact: 'objective.knows_gap', eq: true }, { phase: ['dusk', 'night', 'predawn'] }] }, text: 'You know this gate now, the way Holt told it: the wedge of dark by the north post where the floodlight throws crooked. The dark is on your side for it. Go low and quiet there (HIDE) while the troopers are not roused, and you can be through it before they think to look.' },
        ...
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { not: { fact: 'objective.knows_gap', eq: true } }, { not: { fact: 'reputation.pc.striders', gte: 1 } } ] }, text: 'You do not know this gate\'s rhythm — where its light falls short, or when. Holt might mutter it, in a talkative hour: ask him about the gap. Failing that, good iron would lever the wire-gap wide, or a Strider\'s debt would walk you out.' },

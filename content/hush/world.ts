...
export const ITEMS: ItemDef[] = [
  ...
  { id: 'coin_roll', names: ['coins', 'coin', 'money', 'roll', 'cash', 'scrip'], itemClass: 'coin', portable: true, look: { base: 'A thin roll of cordon scrip and a few hard coins. Not much. Enough to start a conversation.' } },
  ...
  },
  {
    ...
    look: {
      ...
      variants: [
        ...
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { fact: 'objective.knows_gap', eq: true }, { phase: ['dusk', 'night', 'predawn'] }, { fact: 'awareness.cordon_patrol', neq: 'alert' }] }, text: 'You know this gate now, the way Holt told it: the wedge of dark by the north post where the floodlight throws crooked. The dark is on your side for it. Go low and quiet there (HIDE) while the troopers are not roused, and you can be through it before they think to look.' },
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { fact: 'objective.knows_gap', eq: true }, { fact: 'phase.now', eq: 'day' }] }, text: 'You know where the floodlight falls short now — but that wedge of dark is no use to you in broad day. You will have to wait for the light to go before you can slip it (or find another way out).' },
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { not: { fact: 'objective.knows_gap', eq: true } }, { not: { fact: 'reputation.pc.striders', gte: 1 } } ] }, text: 'You do not know this gate\'s rhythm — where its light falls short, or when. Holt might mutter it, in a talkative hour: ask him about the gap. Failing that, good iron would lever the wire-gap wide (if the Greywater has not taken its temper), or a Strider\'s debt would walk you out.' },
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { any: [{ fact: 'possession.pc.iron_knife.condition', eq: 'ore' }, { fact: 'possession.pc.crowbar.condition', eq: 'ore' }] }] }, text: 'The iron you carried went soft in the Greywater — slumped to rotten-red ore in your pack. There is no good metal left on you to lever the wire-gap; that road out is closed, and the Greywater closed it.' },
        { when: { all: [{ fact: 'possession.pc.salvage_core', eq: true }, { fact: 'reputation.pc.striders', gte: 1 }] }, text: 'A Strider owes you a way through, and Mox keeps her debts. Lean on it, and you will be walked out past the wire like baggage.' },
      ],
    ...
    exits: [
      {
        ...
        when: { any: [{ not: { fact: 'flag.intercepted', eq: true } }, { fact: 'flag.intercept_clear', eq: true }, { fact: 'reputation.pc.striders', gte: 1 }, { all: [{ fact: 'objective.knows_gap', eq: true }, { fact: 'flag.hidden', eq: true }, { fact: 'awareness.cordon_patrol', neq: 'alert' }, { phase: ['dusk', 'night', 'predawn'] }] }] },
        blockedText:
